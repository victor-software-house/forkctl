...
- Pull requests and pushes to `main` must pass `verify`, `test:isolated`, and `build` on GitHub's free public-repository `ubuntu-latest` and `macos-latest` hosted runners. Install cargo-deny, cargo-machete, and nextest as GitHub release tarballs, not `cargo:` source builds. Toolchain restore is `mise-action` cache plus `Swatinem/rust-cache`. Pin every external action to a full commit SHA and keep workflow permissions read-only unless a new capability proves otherwise.
