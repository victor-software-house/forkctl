...
- `src/presentation.rs` maps the versioned protocol onto ctl-core semantic documents. ctl-core alone owns help, pretty/JSON/colorless rendering, streams, terminal width, styling, and tables.
- `src/process.rs` is the only production child-process factory and clears Git repository-local hook variables for explicit cwd execution. `run_operator` tees long-running children (publication `git push` / hooks) to stderr in pretty mode; JSON stays captured.
...
- Pull requests and pushes to `main` must pass `verify`, `test:isolated`, and `build` on GitHub's free public-repository `ubuntu-latest` hosted runner. The same gates run on `macos-latest` when a GitHub release is published, or via `workflow_dispatch` — not on every PR. Install cargo-deny, cargo-machete, and nextest as GitHub release tarballs, not `cargo:` source builds. Toolchain restore is `mise-action` cache plus `Swatinem/rust-cache`. Pin every external action to a full commit SHA and keep workflow permissions read-only unless a new capability proves otherwise.
...
- The remote catalog exposes one mounted `fork` file task using `dir = "{{cwd}}"`, `raw_args = true`, exact task tools, `#USAGE mount` equal to `ctl_core::mount_line("fork")`, and `exec forkctl "$@"`. Operators run `mise run fork status` with no `--`. The `--` in the mount is mise's completion bootstrap only.
