...
`mise run verify` is the house gate: rustfmt `--check`, Clippy `-D warnings` (all + pedantic), cargo-nextest, cargo-deny, cargo-machete, and version-pin check. This is a binary crate — there are no rustdoc tests to run. `mise run test` is nextest. `mise run test:isolated` is nextest with one test per process. `mise run deps` is cargo-machete. Lefthook pre-commit formats/lints and scans staged secrets; pre-push runs verify plus a gitleaks history scan. The Rust pin lives in `mise.toml` only — do not add `rust-toolchain.toml`. Containers are reserved for scenarios requiring a distinct OS, network, daemon, or toolchain image; ordinary Git/StGit lifecycle coverage uses real pinned binaries inside `tempfile` sandboxes.
