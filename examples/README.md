...
`fork.json` shows the complete tracked shape after source and tooling patches exist. Source exports are deterministic under `documents.exports`; do not declare per-patch export paths. Persistent ownership uses `scope` globs where `*` stays within a path segment and `**` crosses directories.
